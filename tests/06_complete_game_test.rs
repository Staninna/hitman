...
#[tokio::test]
async fn test_full_game_to_winner() {
    let addr = common::spawn_app().await;
    let client = reqwest::Client::new();
    let num_players = 10;
    ...
    while !game_is_over {
        let players_before_kill = get_players(&client, &addr, &game_code).await;
        let alive_players_before_kill = players_before_kill
            .iter()
            .filter(|p| p.is_alive)
            .collect::<Vec<_>>();

        if alive_players_before_kill.len() <= 1 {
            game_is_over = true;
            continue;
        }

        let killer_ingame_state = alive_players_before_kill.first().unwrap();

        let killer_test_player = test_players
            .iter()
            .find(|p| p.name == killer_ingame_state.name)
            .expect("Killer not found in test players");
        ...
        let kill_response: KillResponsePayload = kill_res.json().await.unwrap();
        game_is_over = kill_response.game_over;
    }
    ...
    assert_eq!(alive_players, 1, "There should be exactly one player alive.");
} 